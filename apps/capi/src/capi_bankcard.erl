...
-spec validate(card_data(), extra_card_data(), session_data() | undefined, payment_system(), validation_env()) ->
    ok | {error, reason()}.
validate(CardData, ExtraCardData, SessionData, PaymentSystem, Env) ->
    MergedCardData = merge_data(CardData, ExtraCardData, SessionData),
    bankcard_validator:validate(MergedCardData, PaymentSystem, Env).
